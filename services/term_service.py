...
from database.db import get_book_by_id, get_term_by_id, get_term_by_name
from database.models import Book, Term
...
class TermService:
    ...
    async def get_definition(
        ...
        *,
        ...
        book: Book = await get_book_by_id(session, id=book_id)

        if term is None:
            logger.debug("Не удалось найти термин `id=%s` в базе данных", term_id)
            raise TermNotFoundByIdError(term_id)

        text, kb = await get_term_info(term=term, book=book, index=index, i18n=i18n)
        if text is None:
            logger.debug(
