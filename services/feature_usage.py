...
async def is_user_allowed_to_use_feature(
    ...
    *,
    ...
    if usage_count is None:
        logger.debug("Не удалось получить количество использований фичи `feature_name`='%s' для пользователя с `user_id`='%d'", feature.name, user_id)
        return True, None  # Разрешаем использование по умолчанию

    if usage_count >= usage_limit:
        logger.warning("Пользователь с `user_id`='%d' превысил лимит использования фичи `feature_name`='%s'", user_id, feature.name)
        return False, None  # Пользователь превысил лимит использования фичи

    logger.debug("Пользователь с `user_id`='%d' может использовать фичу `feature_name`='%s' (использовано %d раз)", user_id, feature.name, usage_count)
