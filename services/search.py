...
async def search_definitions(
    ...
    *,
    ...
    if len(query) < 3:
        ...
        found_terms = await context.execute_search(session, query=query, limit=10) or []
    else:
        ...
        prefix_terms = (
            await prefix_context.execute_search(session, query=query, limit=10) or []
        )

        sim_terms = (
            await similarity_context.execute_search(session, query=query, limit=10)
            or []
        )
