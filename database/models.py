...
class Users(Base):
    ...
    grade: Mapped[UserGrade] = mapped_column(Enum(UserGrade, native_enum=False), nullable=False)
...
class FeatureUsage(Base):
    ...
    user_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("users.user_id"), nullable=False)
    feature_name: Mapped[Feature] = mapped_column(Enum(Feature, native_enum=False), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False,
                                                 server_default=text("TIMEZONE('utc', now())"))
...
class Term(Base):
    ...
    sources: Mapped[list["Source"]] = relationship(back_populates="term", lazy="selectin", cascade="all, delete-orphan")
...
class UserFeedback(Base):
    ...
    definition_id: Mapped[int] = mapped_column(ForeignKey("definitions.id"), nullable=False, index=True)
