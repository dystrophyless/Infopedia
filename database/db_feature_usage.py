...
async def log_feature_usage(
    ...
    *,
    ...
    logger.debug("Использование функции с `feature_name`='%s' было зафиксировано для пользователя с `user_id`='%d'", feature.name, user_id)
...
async def get_users_feature_usage_count(
    ...
    *,
    ...
    result = await session.execute(
        select(func.count(FeatureUsage.id))
        .where(
    ...
    if usage_count is None:
        logger.debug("Не удалось получить количество использований функции с `feature_name`='%s' для пользователя с `user_id`='%d'", feature.name, user_id)
        return None
