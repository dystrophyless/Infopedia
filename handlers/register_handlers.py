...
async def process_channel_link_press(
    ...
    logger.debug(
        "Пользователь с `username`='%s' начал проходить этап подписки в регистрации",
        username,
    )

    if await is_user_followed(bot, callback.from_user.id, channel_id):
        logger.debug(
            "Пользователь с `username`='%s' был подписан на наш канал, ожидаем выбор языка",
            username,
        )
        ...
        logger.debug(
            "Пользователь с `username`='%s' не был подписан на наш канал, ожидем подписку",
            username,
        )
        await callback.answer(text=i18n.get("unsuccessful_membership"), show_alert=True)
...
@router.callback_query(
    F.data.in_([UserGrade.GRADE_11, UserGrade.GRADE_10, UserGrade.GRADE_UNDEFINED]),
...
async def process_failed_to_choose_grade(
    ...
    msg_id = await message.answer(
        text=i18n.get("choose_grade").format(user_language=i18n.get(user_language)),
        reply_markup=build_grade_kb(i18n),
    )
