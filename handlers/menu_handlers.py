...
from keyboards.main_menu import build_profile_menu_kb, build_main_menu_kb, build_buy_subscription_kb
...
async def process_profile_menu_button(
    ...
    username: str = event.from_user.username if event.from_user.username else event.from_user.first_name
    ...
    is_coming_from_language: bool = await state.get_state() == FSMLanguage.choose_language
...
@router.callback_query(F.data == "get_random_term", UserRoleFilter(UserRole.CLIENT, UserRole.ADMIN))
@router.callback_query(F.data == "get_random_term", FeatureAccessFilter(Feature.RANDOM_TERM))
...
async def process_no_access_to_get_random_term(
    ...
    await callback.message.answer(
        text=i18n.get(Feature.RANDOM_TERM.forbidden).format(usage_limit=Feature.RANDOM_TERM.limit),
...
async def process_get_informed_about_roles_button(
    ...
    await callback.message.edit_text(
        ...
        reply_markup=build_buy_subscription_kb(i18n, user_role=user_role, back_to_profile=from_menu)
    )
    await callback.answer()
