...
async def process_find_term_by_definition(
    ...
    reply_markup = (
        build_back_kb(i18n=i18n, callback_data="search") if from_menu else None
    )
...
@router.message(
    StateFilter(FSMSearch.await_definition_to_recognize),
    F.text.regexp(r"^\S+(?:\s+\S+)+$"),
)
...
@router.callback_query(
    F.data.in_(["definition_was_exact", "definition_was_not_exact"]),
    StateFilter(FSMSearch.await_considering_definition),
)
...
async def process_suggestion_positive_reply(
    ...
    logger.debug(
        "Пользователь с `username`='%s' предложил следующий термин: %s",
        username,
        suggested_term,
    )
