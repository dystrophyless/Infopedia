...
class RegistrationMiddleware(BaseMiddleware):
    async def __call__(
        ...
        if user_row is None:
            logger.debug("Данные о пользователе не удалось получить из базы данных, определяем роль пользователя, передаём в контекст, устанавливаем состояние регистрации, переходим в следующий \"обработчик\"")
            ...
            logger.debug("Роль пользователя: %s была передана в контекст", user_role)
